...
{ name: "James Anderson", basePrice: 120000000, role: "Bowler", nationality: "Foreign", image: "james_anderson.jpg" },
    { name: "Prithvi Shaw", basePrice: 70000000, role: "Batsman", nationality: "Indian", image: "prithvi_shaw.jpg" },
    { name: "Sarfaraz Khan", basePrice: 70000000, role: "Batsman", nationality: "Indian", image: "sarfaraz_khan.jpg" },
...
nextPlayerIndex = localStorage.getItem("nextPlayerIndex") ? parseInt(localStorage.getItem("nextPlayerIndex")) : 0;

// Function to start the auction for the next player
function startNextAuctionPlayer() {
    
           if (currentAuctionIndex < players.length) {
        const player = players[currentAuctionIndex];
        startAuctionForPlayer(player, currentAuctionIndex);
    } else {
        announce("All players have been auctioned.");
        clearAuctionData(); // End the auction when all players are done
    }
}
    ...
    const auctionInterval = setInterval(() => {
        if (Date.now() > bidTimeout) {
            ...
            if (highestBidder) {
                ...
                player.status = "sold";  // Update the player's status to 'sold'
                markPlayerStatus(playerIndex, "Sold", "green"); // Mark player as Sold
            } else {
                announce(`No bids for ${player.name}`);
                player.status = "unsold"; // Update the player's status to 'unsold'
                markPlayerStatus(playerIndex, "Unsold", "red"); // Mark player as Unsold
            }

            renderAuctionPlayersList(); // Update player list
            players.splice(playerIndex, 1); // Remove the player from the list
             
            updateCurrentAuctionIndex(playerIndex + 1);// Update the player index for the next auction
            saveSoldPlayersToLocalStorage(); // Save the sold players
...
// Variable to track the starting player index
...
function renderAuctionPlayersList() {
    ...
    const remainingPlayers = players.slice(currentAuctionIndex);
...
function clearAuctionData() {
    ...
    localStorage.removeItem("currentAuctionIndex"); // Reset player index as well
