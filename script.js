...
function saveUserTeamToLocalStorage() {
    if (userTeam) {
        const teamData = {
            ...
            squad: userTeam.squad, // Save only the squad and basic properties
        };
        localStorage.setItem("userTeam", JSON.stringify(teamData));
    }
}
...
function loadUserTeamFromLocalStorage() {
    ...
    if (storedTeam) {
        ...
        userTeam = new Team(
            parsedTeam.name,
            parsedTeam.budget,
            parsedTeam.squad || [] // Use the saved squad or an empty array
        );

        // Ensure squad properties are correctly updated
        userTeam.foreignPlayers = parsedTeam.squad.filter(player => player.nationality === 'Foreign').length;
        userTeam.batsmen = parsedTeam.squad.filter(player => player.role === 'Batsman').length;
        userTeam.allRounders = parsedTeam.squad.filter(player => player.role === 'All-Rounder').length;
        userTeam.bowlers = parsedTeam.squad.filter(player => player.role === 'Bowler').length;
        userTeam.wicketkeepers = parsedTeam.squad.filter(player => player.role === 'Wicketkeeper').length;

        document.getElementById("team-selection").style.display = "none";
        document.getElementById("auction-section").style.display = "block";
        document.getElementById("user-bid-btn").style.display = "block";
        startNextAuctionPlayer(); // Start the auction for the next player
    } else {
...
function startAuctionForPlayer(player, playerIndex) {
    ...
    const auctionInterval = setInterval(() => {
        if (Date.now() > bidTimeout) {
            ...
            // Check if highestBidder is valid
            if (highestBidder) {
                // If highestBidder is not the user team, add the player and update sold player
                if (highestBidder !== userTeam) {
                    ...
                    updateSoldPlayer(highestBidder, player);  // Update sold player for other teams
                } else {
                    // If highestBidder is userTeam, add player to userTeam and update sold player
                    userTeam.addPlayer(player, player.currentPrice);
                    announce(`${userTeam.name} won ${player.name} for ₹${player.currentPrice / 1e7} crore`, userTeam.name.toLowerCase());
                    updateSoldPlayer(userTeam, player);  // Update sold player for userTeam
                }

                // Update player status to sold and mark player status
                player.status = "sold";
        ...
        // Only non-user teams participate in automatic bidding
    ...
    }, 1000);  // Closing the setInterval properly



    // User bid button functionality
    document.getElementById("user-bid-btn").onclick = function() {
        ...
        if (userTeam.budget >= newBidPrice) {
            ...
            highestBidder = userTeam;  // Set user as the highest bidder
            lastBidTime = Date.now();
            ...
            bidTimeout = Date.now() + 30000;  // Reset the timeout after user bid
        } else {
